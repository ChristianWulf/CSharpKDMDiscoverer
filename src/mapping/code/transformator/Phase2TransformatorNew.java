...
import de.cau.chw.transformation.external.UnknownEntityListener;

public class Phase2TransformatorNew extends AbstractTransformator implements IKDMMapper<CompilationUnit> {

	private static final Logger							LOGGER							= new Logger(
			Phase2TransformatorNew.class);
	...
	public final List<UnknownEntityListener>			unknownEntityListeners		= new LinkedList<UnknownEntityListener>();

	private final Set<ExternalEntity>					externalEntities			= new HashSet<ExternalEntity>();
	private final Map<ResolutionMarker, ExternalEntity>	markers						= new HashMap<ResolutionMarker, ExternalEntity>();

	public Phase2TransformatorNew(final GenericLanguageUnitCache genericLanguageUnitCache,
		...
		unknownEntityListeners.add(new UnknownEntityListener() {
			@Override
			public void unknownEntityDetected(final ExternalEntity externalEntity, final ResolutionMarker marker) {
				externalEntities.add(externalEntity);
				markers.put(marker, externalEntity);
			}
		});
	}
	...
	public void transform(final CommonTree node, final SourceFile sourceFile) {
		...
		try {
			...
			LOGGER.error("Exception while processing '" + sourceFile.getPath() + "'", e);
			throw e;
		}
	...
	protected Object walk(final Tree node, final CodeItem parent) throws TypeNotFoundException {
		...
		switch (commonTreeNode.getType()) {
			...
			case CSharp4AST.USING_ALIAS_DIRECTIVE:
				...
				// FIXME a using directive can not only be a namespace, but also a type
				Namespace namespaceOrType = searcher.findNamespaceFromGlobals(usingAliasNamespace,
						NamespaceSearcher.nsNullListener);
				// if not found, save for later processing
				if (namespaceOrType == null) {
					fireUnknownEntityDetected(new ExternalEntity(usingAliasNamespace), new ResolutionMarker(
							Imports.class, compilationUnit));
					break;
				}
				Imports aliasImports = KDMElementFactory.createImports(parent, namespaceOrType);
				/*
			...
			case CSharp4AST.USING_NAMESPACE_DIRECTIVE:
				...
				Namespace foreignNamespace;
				{
					...
					if (foreignNamespace == null) {
						fireUnknownEntityDetected(new ExternalEntity(usingNamespace), new ResolutionMarker(
								Imports.class, compilationUnit));
						break;
					} else {
						LOGGER.fine("found namespace: " + foreignNamespace);
					}
				}
			...
			case CSharp4AST.STRUCT:
				...
				LOGGER.warning("LOGGER.unsupported: struct is not yet supported");
				declarations.pop();
				return structType;
			...
			case CSharp4AST.ENUM:
				...
				if (enumBaseType != null) {
					...
					// TODO
					LOGGER.unsupported("Super types for ENUMs are not yet supported.");
				}
			...
			case CSharp4AST.EVENT_VARS_DECL:
				// do not process children of type TYPE; it's resolved before or after (see grammar)
				for (int i = 0; i < commonTreeNode.getChildCount(); i++) {
					Tree child = commonTreeNode.getChild(i);
				...
				break;
			case CSharp4AST.EVENT_INTERFACE_DECL:
				return declaratorTransformer.transformEventInterface(commonTreeNode, parent, declarations);
			...
			case CSharp4AST.MEMBER_NAME:
				...
				if (typeName.size() > 1) {
					LOGGER.warning("LOGGER.unsupported: member_name with more than one IDENTIFIER is not yet supported: "
							+ typeName);
				}
			...
			case CSharp4AST.MEMBER_ACCESS:
				...
				return accessList;
			default:
				LOGGER.warning("Skip; Process in next phase: " + commonTreeNode.getText());
				break;
		}
		return null;
	}

	void fireUnknownEntityDetected(final ExternalEntity externalEntity, final ResolutionMarker resolutionMarker) {
		for (UnknownEntityListener e : unknownEntityListeners) {
			e.unknownEntityDetected(externalEntity, resolutionMarker);
		}
	}
		...
		if (typeName.size() == 1 && "object".equals(extendsImplementsTypeName)) {
			...
			try {
				...
				if (type.eContainer() == null) {
					...
					fireUnknownEntityDetected(new ExternalEntity(typeName), new ResolutionMarker(
							AbstractCodeRelationship.class, parent));
	...
	public Map<ResolutionMarker, ExternalEntity> getMarkers() {
		return markers;
	}
