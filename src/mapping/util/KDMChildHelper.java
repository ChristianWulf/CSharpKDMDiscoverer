...
public class KDMChildHelper {
	...
	public static <T> T getChildItemByName(final String ident, final CodeItem parent,
			final Class<T> clazz) {
	...
	public static <T> T getChildItemByName(final String ident,
	...
	public static <T> T getChildItemByType(
			final List<? extends AbstractCodeElement> elements, final Class<T> clazz) {
		for (AbstractCodeElement ace : elements) {
			if (clazz.isAssignableFrom(ace.getClass()))
				return (T) ace;
		}
		return null;
	}
	...
	public static MethodUnit getChildMethodByName(final String identifier,
			final CodeItem parent) {
		...
		for (AbstractCodeElement ace : elements) {
			...
			if (ace instanceof MethodUnit) {
				...
				String key = KeyStringHelper.buildSignatureKey(methodName,
						signature.getParameterUnit());
	...
	public static <T> T getAbstractCodeElementByName(
			final List<? extends AbstractCodeElement> elements, final String name,
			final Class<T> clazz) {
		for (AbstractCodeElement el : elements) {
			if (clazz.isAssignableFrom(el.getClass()) && name.equals(el.getName()))
				return (T) el;
	...
	public static CompilationUnit getCompilationUnit(final SourceFile sourceFile,
			final CodeModel internalCodeModel) {
	...
	@SuppressWarnings("unchecked")
	public static <T, C> List<C> filterChildrenByType(final List<T> elements,
			final Class<C> clazz) {
		List<C> filteredElements = new LinkedList<C>();
		for (T ace : elements) {
			if (clazz.isAssignableFrom(ace.getClass()))
				filteredElements.add((C) ace);
		}
