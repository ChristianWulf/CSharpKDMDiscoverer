...
namespace System
{
	...
	public struct RuntimeTypeHandle : ISerializable
	{
		...
		internal unsafe static Type GetTypeHelper(Type typeStart, Type[] genericArgs, IntPtr pModifiers, int cModifiers)
		{
			...
			if (cModifiers > 0)
			{
				...
				for (int i = 0; i < cModifiers; i++)
				{
					...
					{
						if ((byte)Marshal.ReadInt32((IntPtr)((void*)value), i * 4) == 16)
						{
							type = type.MakeByRefType();
						}
						else
						{
							if ((byte)Marshal.ReadInt32((IntPtr)((void*)value), i * 4) == 29)
							{
		...
		[ForceTokenStabilization, SecuritySafeCritical]
		[MethodImpl(MethodImplOptions.InternalCall)]
		...
		{
			return !RuntimeTypeHandle.HasElementType(type) && RuntimeTypeHandle.HasInstantiation(type);
		}
		[SecurityCritical]
		...
		{
			string result = null;
			RuntimeTypeHandle.ConstructName(this.GetNativeHandle(), nameSpace, fullInst, assembly, JitHelpers.GetStringHandleOnStack(ref result));
			return result;
		}
		[SecurityCritical]
		[MethodImpl(MethodImplOptions.InternalCall)]
		...
		internal static RuntimeType GetTypeByName(string name, bool throwOnError, bool ignoreCase, bool reflectionOnly, ref StackCrawlMark stackMark, bool loadTypeFromPartialName)
		{
			if (name != null && name.Length != 0)
			{
				...
				RuntimeTypeHandle.GetTypeByName(name, throwOnError, ignoreCase, reflectionOnly, JitHelpers.GetStackCrawlMarkHandle(ref stackMark), loadTypeFromPartialName, JitHelpers.GetObjectHandleOnStack<RuntimeType>(ref result));
				return result;
			}
			if (throwOnError)
			{
		...
		{
			return RuntimeTypeHandle.IsCollectible(this.GetNativeHandle());
		}
		[MethodImpl(MethodImplOptions.InternalCall)]
		...
		internal static RuntimeType GetGenericTypeDefinition(RuntimeType type)
		{
			...
			if (RuntimeTypeHandle.HasInstantiation(runtimeType))
			{
