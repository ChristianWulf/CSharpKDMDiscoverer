...
namespace System
{
	[ClassInterface(ClassInterfaceType.None), AttributeUsage(AttributeTargets.All, Inherited = true, AllowMultiple = false), ComDefaultInterface(typeof(_Attribute)), ComVisible(true)]
	[Serializable]
	public abstract class Attribute : _Attribute
	{
		...
		private static PropertyInfo GetParentDefinition(PropertyInfo property)
		{
			...
			if (runtimeMethodInfo != null)
			{
				...
				if (runtimeMethodInfo != null)
				{
					return runtimeMethodInfo.DeclaringType.GetProperty(property.Name, BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, property.PropertyType);
				}
			}
			return null;
		}
		...
		private static void AddAttributesToList(List<Attribute> attributeList, Attribute[] attributes, Dictionary<Type, AttributeUsageAttribute> types)
		{
			for (int i = 0; i < attributes.Length; i++)
			{
				...
				{
					if (attributeUsageAttribute.Inherited && attributeUsageAttribute.AllowMultiple)
					{
						attributeList.Add(attributes[i]);
					}
				}
		...
		public override bool Equals(object obj)
		{
			...
			{
				object thisValue = ((RtFieldInfo)fields[i]).InternalGetValue(this, false, false);
		...
		public override int GetHashCode()
		{
			...
			for (int i = 0; i < fields.Length; i++)
			{
				object obj2 = ((RtFieldInfo)fields[i]).InternalGetValue(this, false, false);
